...
void ModularTransport::Mainloop(){
    ...
    while (!this->scheduler->is_empty()){
        ...
        EventProcessorOutput* epout = new EventProcessorOutput();
        epout->ctx = ctx;
        ...
        std::cout <<"Done with event processors "<< std::endl;
        this->ctx_table[e->flowId] = epout->ctx;
        for (auto newEvent : epout->events)
        {
        // here, since events array has been generated by event processor, it should
        //  only contain OUTGOING events
        switch (newEvent->subtype){
            case PROG_EVENT:
                scheduler->enqueue_event(e->flowId,newEvent);
                break;
            case NET_EVENT:
                // if OUTGOING and NET_EVENT, must go to TXNet to be sent to network layer 
                SendPacket(txnet->get_next_packet(newEvent),cursaddr,curdaddr);
                std::cout <<"sending out a packet "<< std::endl;
                break;
            case APP_EVENT:
                // if OUTGOING and APP_EVENT, must go to TXApp to be sent to application layer
                txapp->send_app_event(newEvent);
                break;
            default:
    ...
    std::cout << "returning from mainloop" << std::endl;
}
